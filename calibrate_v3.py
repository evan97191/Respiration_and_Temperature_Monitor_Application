    ...
    try:
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith("POINTS_VIS"):
                ...
                vis_updated = True
                print(f"正在更新 POINTS_VIS...")
            elif line.strip().startswith("POINTS_IR"):
                lines[i] = new_ir_line
                ir_updated = True
                print(f"正在更新 POINTS_IR...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if res < 0: raise Exception("uvc_init error")
        res = libuvc.uvc_find_device(ctx, byref(dev), config.THERMAL_VID, config.THERMAL_PID, 0)
        if res < 0: raise Exception("uvc_find_device error")
        res = libuvc.uvc_open(dev, byref(devh))
        if res < 0: raise Exception("uvc_open error")
        print("熱像儀已連接。")
        ...
        if res < 0: raise Exception("uvc_start_streaming error")
    except Exception as e:
