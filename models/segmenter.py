...
class UNetSegmenter:
    ...
    @staticmethod
    def overlay_mask(image_np, mask_np, color=None, alpha=0.5):
        ...
        if image_np.shape[:2] != mask_np.shape:
            ...
            try:
                mask_np_resized = cv2.resize(
                    mask_np.astype(np.uint8), (image_np.shape[1], image_np.shape[0]), interpolation=cv2.INTER_NEAREST
                )
        ...
        try:
            ...
            overlay[foreground] = cv2.addWeighted(
                image_np[foreground], 1 - alpha, np.full_like(image_np[foreground], color_np), alpha, 0
            )
        except Exception as e:
    ...
    def extract_foreground(image_np, mask_np):
        ...
        if image_np.shape[:2] != mask_np.shape:
            ...
            try:
                mask_np_resized = cv2.resize(
                    mask_np.astype(np.uint16), (image_np.shape[1], image_np.shape[0]), interpolation=cv2.INTER_NEAREST
                )
                mask_np = mask_np_resized
            except Exception as e:
