    ...
    def _monitor_loop(self):
        try:
            with open(self.output_csv, mode="w", newline="") as f:
                ...
                # csv 標頭
                writer.writerow(
                    ["Timestamp", "CPU_Usage(%)", "GPU_Usage(%)", "RAM_Used(MB)", "Power(mW)", "GPU_Temp(C)"]
                )
                    ...
                    try:
                        writer.writerow(
                            [
                                ...
                                round((self.jetson.memory["RAM"]["used"] / 1024.0), 2)
                                if "RAM" in self.jetson.memory
                                else 0,  # 記憶體使用量 (KB 轉 MB)
                                self.jetson.power["tot"]["power"] if "tot" in self.jetson.power else 0,  # 總功耗
                                self.jetson.temperature["gpu"]["temp"]
